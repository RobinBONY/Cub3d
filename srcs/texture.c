...
/*   Updated: 2022/10/14 10:25:07 by alakhdar         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	init_textures(t_game *game)
{
	...
	if (game->textures == NULL)
		return (1);
	error |= store_textures_on_image(game, game->textures->n_texture,
			game->texture_name[0]);
	error |= store_textures_on_image(game, game->textures->s_texture,
			game->texture_name[1]);
	error |= store_textures_on_image(game, game->textures->e_texture,
			game->texture_name[2]);
	error |= store_textures_on_image(game, game->textures->w_texture,
			game->texture_name[3]);
	if (error == 1)
...
void	calculate_stripe(t_raycasting *raycasting, t_game *game)
{
	...
	//return (draw_start/draw_end/line_height) ou les mettre dans une des struct
}
...
double	calculate_texturing(t_raycasting *raycasting, t_textures *texture)
{
	...
	if (ray->side == 0)
		...
		wallx = pos_x + perpWallDist * ray_dirx;
	wallx -= floor(wallx);
